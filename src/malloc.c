...
{
	(*elem) = sbrk(0);
	if ((*elem) == (void*)-1 || sbrk(DATA_BLOCK_SIZE) == (void*)-1)
		return (1);
	(*elem)->empty = 0;
	(*elem)->size_blk = size;
	(*elem)->next = NULL;
...
void *check_free_block(data_info_t **data, size_t size)
{
	...
	if (tmp->size_blk - size >= ((size_t)DATA_BLOCK_SIZE + 8)) {
		new = (void *)tmp + (DATA_BLOCK_SIZE + size);
		new->empty = 1;
		new->size_blk = align(tmp->size_blk - size - DATA_BLOCK_SIZE);
		new->next = tmp->next;
		new->prev = tmp;
		if (new->next != NULL)
			new->next->prev = new;
...
/*	if (tmp->size_blk - size > ((size_t)DATA_BLOCK_SIZE * 2)) {
		new = (void*)tmp + (DATA_BLOCK_SIZE + size);
		new->empty = 1;
		new->size_blk = tmp->size_blk - size - DATA_BLOCK_SIZE;
		printf("%d\n", new->size_blk);
		new->next = tmp->next;
		new->prev = tmp;
		if (new->next != NULL)
